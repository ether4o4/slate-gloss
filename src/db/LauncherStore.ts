...
export const recordRecent = (state: LauncherState, pkg: string): LauncherState => {
  const recents = [pkg, ...state.recents.filter(p => p !== pkg)].slice(0, MAX_RECENTS);
...
export const addToDesktop = (
  ...
  if (state.desktop.some(d => d.packageName === pkg)) return state;
  const taken = new Set(state.desktop.map(d => `${d.col},${d.row}`));
  for (let col = 0; col < cols; col++) {
    for (let row = 0; row < rows; row++) {
      if (!taken.has(`${col},${row}`)) {
        return {...state, desktop: [...state.desktop, {packageName: pkg, col, row}]};
      }
    }
  }
  // Grid full — stack at 0,0.
  return {...state, desktop: [...state.desktop, {packageName: pkg, col: 0, row: 0}]};
};
...
export const moveDesktopIcon = (
  ...
  const desktop = state.desktop.map(d => {
    if (d.packageName === pkg) return {...d, col, row};
...
export const recycleDesktopIcon = (state: LauncherState, pkg: string): LauncherState => ({
  ...state,
...
export const restoreFromRecycle = (
  ...
  const without = {...state, recycle: state.recycle.filter(r => r.packageName !== pkg)};
  return addToDesktop(without, pkg, cols, rows);
};

export const emptyRecycle = (state: LauncherState): LauncherState => ({...state, recycle: []});
